...
class Coin_small(pygame.sprite.Sprite):
    def __init__(self):
        super().__init__()
        self.image = pygame.transform.scale(pygame.image.load("coin1.png"), (30, 30))
        self.rect = self.image.get_rect()
        self.rect.center = (random.randint(40, SCREEN_WIDTH -40), random.randint(0, SCREEN_HEIGHT - 150))
    def move(self):
        self.rect.move_ip(0, 3)
...
bounties_s = pygame.sprite.Group()
bounties.add(C1)
bounties_s.add(CM1)
...
while True:
    ...
    for event in pygame.event.get():
        ...
        if event.type == COIN_SPAWN and COINS_NUM <= 1:
            ...
            all_sprites.add(C1)
            COINS_NUM += 1   
        if event.type == COIN_SPAWN and COINS_NUM1 <= 1:
            CM1 = Coin_small()
            bounties_s.add(CM1)
            all_sprites.add(CM1)
            COINS_NUM1 += 1     
    ...
    elif pygame.sprite.spritecollide(P1, bounties, 1):
        C1.contact()
        COINS_NUM -= 1
    elif pygame.sprite.spritecollide(P1, bounties_s, 1):
        CM1.contact()
        COINS_NUM -= 1
